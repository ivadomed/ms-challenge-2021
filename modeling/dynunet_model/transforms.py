...
from monai.transforms import (CastToTyped,
                              ...
                              NormalizeIntensity, NormalizeIntensityd, RandCropByPosNegLabeld,
...
def get_task_transforms(mode, patch_size, pos_sample_num, neg_sample_num, num_samples):
    if mode != "test":
        keys = ["image_ses1", "image_ses2", "label_c"]
    else:
        keys = ["image_ses1", "image_ses2"]

    load_transforms = [
        LoadImaged(keys=keys), 
        EnsureChannelFirstd(keys=keys),]
        # CropForegroundd(keys=keys, source_key="image_ses2"),
        # NormalizeIntensityd(keys=keys, nonzero=True, channel_wise=True)]
        # trying normalization here itself as the last step of PreproAnistropic is intensity norm.
    
    # 2. sampling
    sample_transforms = [
        PreprocessAnisotropic(
            keys=keys,
            clip_values=[0.0, 0.0], # clip_values[task_id],
            pixdim=[0.75, 0.75, 0.75],
            normalize_values=[0.0, 0.0],  # normalize_values[task_id],
            model_mode=mode,
        ),
    ]
    # after this point, image_ses1 and image_ses2 are concatenated to get "image"
    # (happens in PreprocessAnisotropic) 
    ...
    if anisotrophy_flag:
        for image_c in image:
            ...
            for i in range(image_c.shape[-1]):
                ...
                image_c_2d_slice = resize(image_c_2d_slice, shape[:-1], order=3, mode="edge", cval=0, clip=True, anti_aliasing=False,)
                resized_slices.append(image_c_2d_slice)
            resized = np.stack(resized_slices, axis=-1)
            resized = resize(resized, shape, order=0, mode="constant", cval=0, clip=True, anti_aliasing=False,)
            resized_channels.append(resized)
    else:
        for image_c in image:
            resized = resize(image_c, shape, mode="edge", cval=0, clip=True, anti_aliasing=False,)
            resized_channels.append(resized)
    ...
    if anisotrophy_flag:
        ...
        for class_ in range(1, int(n_class) + 1):
            for depth_ in range(depth):
                ...
                resized_2d = resize(mask.astype(float), shape_2d, order=1, mode="edge", cval=0, clip=True, anti_aliasing=False,)
                reshaped_2d[:, :, depth_][resized_2d >= 0.5] = class_
        for class_ in range(1, int(n_class) + 1):
            mask = reshaped_2d == class_
            resized = resize(mask.astype(float), shape, order=0, mode="constant", cval=0, clip=True, anti_aliasing=False,)
            reshaped[resized >= 0.5] = class_
    else:
        for class_ in range(1, int(n_class) + 1):
            mask = label[0] == class_
            resized = resize(mask.astype(float), shape, order=1, mode="edge", cval=0, clip=True, anti_aliasing=False,)
            reshaped[resized >= 0.5] = class_
...
def recovery_prediction(prediction, shape, anisotrophy_flag):
    ...
    if anisotrophy_flag:
        ...
        for class_ in range(1, n_class):
            ...
            resized_d = resize(mask.astype(float), (h, w, d), order=0, mode="constant", cval=0, clip=True, anti_aliasing=False,)
            reshaped_d[class_][resized_d >= 0.5] = 1

        for class_ in range(1, n_class):
            for depth_ in range(d):
                mask = reshaped_d[class_, :, :, depth_] == 1
                resized_hw = resize(mask.astype(float), shape[1:-1], order=1, mode="edge", cval=0, clip=True, anti_aliasing=False,)
                reshaped[class_, :, :, depth_][resized_hw >= 0.5] = 1
    else:
        for class_ in range(1, n_class):
            mask = prediction[class_] == 1
            resized = resize(mask.astype(float), shape[1:], order=1, mode="edge", cval=0, clip=True, anti_aliasing=False,)
...
class PreprocessAnisotropic(MapTransform):
    ...
    def __init__(self, keys, clip_values, pixdim, normalize_values, model_mode,) -> None:
        super().__init__(keys)
        self.keys = keys
    ...
    def __call__(self, data):
        # load data
        d = dict(data)
        image1, image2 = d["image_ses1"], d["image_ses2"]
        image = np.concatenate([image1, image2], axis=1)
        print(f"image shape: {image.shape}")
        image_spacings = d["image_ses2_meta_dict"]["pixdim"][1:4].tolist()  
        # same for both images because they are co-registered 

        if "label_c" in self.keys:
            label = d["label_c"]
            label[label < 0] = 0
