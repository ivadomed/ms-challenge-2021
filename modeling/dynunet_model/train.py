...
save_dir = "/home/GRAMES.POLYMTL.CA/u114716/ms-challenge-2021/saved_models/nnunet/"
...
def validation(args):
    ...
    deep_supervision_num = args.deep_supervision_num
    ...
    val_output_dir = save_dir + "run_fold-{}_{}/".format(args.fold, args.expr_name)
    ...
    val_loader = get_data(args, mode="validation")
    # TODO: fix arguments
    net = get_dynunet(
        kernels=None, 
        strides=None, 
        deep_supervision_num=deep_supervision_num,  
        pretrain_path=val_output_dir, 
    ...
    num_classes = 2     # one for bg and one for fg i.e. bg=0, fg=1
    ...
    evaluator = DynUNetEvaluator(
        ...
        inferer=SlidingWindowInferer(
            roi_size=patch_size, sw_batch_size=sw_batch_size, overlap=eval_overlap, mode=window_mode,
        ),
        postprocessing=None,
        key_val_metric={
            "val_mean_dice": MeanDice(include_background=False, output_transform=from_engine(["pred", "label_c"]),)
        },
...
def train(args):
    ...
    fold = args.fold
    val_output_dir = save_dir + "run_fold-{}_{}/".format(args.fold, args.expr_name)
    log_filename = "nnunet_fold-{}.log".format(fold)
    ...
    val_loader = get_data(args, mode="validation")
    train_loader = get_data(args, batch_size=train_batch_size, mode="train")
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=learning_rate, momentum=0.99, weight_decay=3e-5, nesterov=True,)
    ...
    evaluator = DynUNetEvaluator(
        ...
        key_val_metric={
            "val_mean_dice": MeanDice(
                ...
                output_transform=from_engine(["pred", "label_c"]),
            )
        },
...
if __name__ == "__main__":
    ...
    parser.add_argument("-fold", "--fold", type=int, default=0, help="0-4")
    parser.add_argument("-root_dir", "--root_dir", type=str,
        default="/home/GRAMES.POLYMTL.CA/u114716/datasets/ms-challenge-2021_preprocessed_clean/",
        help="dataset path",
    )
    parser.add_argument("-expr_name", "--expr_name", type=str,
        default="expr",
        help="the suffix of the experiment's folder",
    )
    parser.add_argument("-train_num_workers", "--train_num_workers", type=int, default=4, 
    ...
    parser.add_argument("-ps", "--patch_size", type=int, default=128, help="input patch sizes",)
    parser.add_argument("-bs", "--batch_size", type=int, default=8, help="batch size",)
    parser.add_argument("-dsn", "--deep_supervision_num", type=int, default=3, help="number of DeepSupervision heads",)
